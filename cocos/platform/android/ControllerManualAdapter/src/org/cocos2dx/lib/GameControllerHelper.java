...
public class GameControllerHelper {
    ...
    public GameControllerHelper(GameControllerActivity activity){
        ...
        // 新增對應表
        ControllerKeyMap.put(188, GameControllerDelegate.BUTTON_B);
        ControllerKeyMap.put(189, GameControllerDelegate.BUTTON_A);
        ControllerKeyMap.put(190, GameControllerDelegate.BUTTON_Y);
        ControllerKeyMap.put(191, GameControllerDelegate.BUTTON_X);
        
        
        ControllerKeyMap.put(192, GameControllerDelegate.BUTTON_LEFT_SHOULDER);		//L1
        ControllerKeyMap.put(193, GameControllerDelegate.BUTTON_LEFT_TRIGGER);    	//L2
        ControllerKeyMap.put(194, GameControllerDelegate.BUTTON_RIGHT_SHOULDER);	//R1
        ControllerKeyMap.put(195, GameControllerDelegate.BUTTON_RIGHT_TRIGGER);   	//R2
        
        ControllerKeyMap.put(196, GameControllerDelegate.BUTTON_START);
        ControllerKeyMap.put(197, GameControllerDelegate.BUTTON_SELECT);             
    ...
    public boolean dispatchGenericMotionEvent(MotionEvent event) {      
        ...
        Log.d("debug","=== GameControllerHelper dispatchGenericMotionEvent eventSource=" + eventSource);
    ...
    public boolean dispatchKeyEvent(KeyEvent event) {
        ...
        Log.d("debug","=== GameControllerHelper dispatchKeyEvent eventSource=" + eventSource + " keyCode=" + keyCode + " controllerKey" + controllerKey);
        
        if (((eventSource & InputDevice.SOURCE_GAMEPAD) == InputDevice.SOURCE_GAMEPAD)
                || ((eventSource & InputDevice.SOURCE_JOYSTICK) == InputDevice.SOURCE_JOYSTICK) ) 
        {
        	Log.d("debug","=== GameControllerHelper dispatchKeyEvent 偵測到有搖桿");
            ...
            if (controllerKey == 0) {
            	...
            	Log.d("debug","=== GameControllerHelper dispatchKeyEvent 沒支援該按鍵喔, 要退出了 keyCode:" + keyCode);
            ...
            if (action == KeyEvent.ACTION_DOWN) {
                handled = true;
                Log.d("debug","=== GameControllerHelper dispatchKeyEvent 按下 keyCode:" + keyCode + " controllerKey" + controllerKey);
                GameControllerAdapter.onButtonEvent(deviceName, deviceId, controllerKey,true, 1.0f, false);
            }else if (action == KeyEvent.ACTION_UP) {
                handled = true;
                Log.d("debug","=== GameControllerHelper dispatchKeyEvent 放開 keyCode:" + keyCode + " controllerKey" + controllerKey);
    ...
    public static void receiveExternalKeyEvent(int deviceId,int externalKeyCode,boolean receive) {
   		...
   		Log.d("debug","=== GameControllerHelper receiveExternalKeyEvent deviceId=" + deviceId + " externalKeyCode=" + externalKeyCode + " receive=" + receive);
    
        if (receive) {
            if (mControllerExtendKey.get(deviceId) == null) {
            	Log.d("debug","=== GameControllerHelper receiveExternalKeyEvent 11111");
            ...
            if (mControllerExtendKey.get(deviceId) != null) {
            	Log.d("debug","=== GameControllerHelper receiveExternalKeyEvent 222222");
